...
class LazySegTree:
    ...
    def __updateRangeImpl(self, node, st, en, left, right, diff):
        ...
        if left <= st and en <= right:
            self.tree[node] ^= xorSelf(diff, en - st + 1)
            if st != en:
    ...
    def __updateLazy(self, node, st, en):
        ...
        self.tree[node] ^= xorSelf(self.lazy[node], en - st + 1)
        if st != en:
    ...
    def getValue(self, index):
        return self.__sumImpl(1, 0, self.arrLen - 1, index, index)
...
def solve(M, arr):
    lst = LazySegTree(arr)
    for _ in range(M):
        cmd, *params = map(int, readline().split())
        if cmd == 1:
            lst.updateRange(*params)
        elif cmd == 2:
            print(lst.getValue(*params))
